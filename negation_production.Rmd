...
<!-- TO DO
+ Put error bars on developmental plots (bootstrap)
...
+ Add individual plots for kids in Providence and Manchester, 
...
<<<<<<< HEAD
<!-- Outline
- Negation is an important abstract logical concept. How does it emerge in humans? Possible role of language.
  + In a language like English several morphemes encode negation (no, not, n't). What are their developmental profiles? (Study 1)
  + Do children start by composing negation in a limited fashion? For example, pre-sententially? (Study 2)
  + How do children start composing negation with other words they know and how do these compositions develop? (Study 3)
--->

<!-- Start paragraph
=======

>>>>>>> 7a88ccf17a59fcd71941a9cbde7b9f5df37766ea
...
Possible origins: 1. Metalinguistic and conceptual categorization (labeling) 2. Higher Perceptual (existence in "there is a" or with PPs like "it's in the bucket") 3. affective (rejections, wants, likes) 4. Motor (prohibitions). Two hypothesis: 1. unified concept of negation that is incorporated to operate on different representations and 2. heterogeneous concepts of negation that develop independently but can be generalized into a single concept.
...
<<<<<<< HEAD
Study 1: previous studies have suggested a non-unified or stage-like development. This study shows that even though a piecemeal and slow development of abstract negation is possible, the production data is not conclusive. We show that the proposed stages of development in production are either child-specific and not found when we aggregate data across children, or they are a phenomena likely caused by production limitations.

Study 2: delves deeper into claims of non-adult like productions

Study 3: investigates early constructions in parent's and children's speech as a source of insight into the possible origins of linguistic negation. We track constructions that convey existence of an object or its location (there is no book here, it's in the car, ...), constructions that classify and label objects revealing their identity (that's not a car, it's not a book), constructions that regulate action (don't do X), constructions that convey desires (I don't want X), ...

Origins Hypotheses:
1. Perceptual Origin: express nonexistence of an expected object or action -- no more X, there is no X, it's not PP, allgone?, it's not spinning, etc.

2. Conceptual Origin: classify objects or actions: this is not a car, that's not running, ...

3. Preferential Origin: it is used to convey desires and preferences -- combinations of negation with "want", "like", 

4. Actional Origin: used in imperatives or with ability modals: "don't do X", "I cant do X", ...
(problem: can has other uses like: you can't play with that you may break it.)
5. Epistemic: I don't know?
...
## Previous Production Studies of Negation in Children
...
<<<<<<< HEAD
We also looked at individual children's production of negative morphemes. We selected the following corpora based on the fact that they had available longitudinal data in the 1-3 years age range: Brown [@brown1973first], Bloom [@bloom1970; @bloom1974imitation], Braunwald [@braunwald1971mother], Suppes [@suppes1974semantics], Sachs [@sachs1983talking], Manchester [@theakston2001role], MPI-EVA-Manchester and Thomas [@lieven2009two], Providence [@demuth2006word], and Lara [@rowland2007explaining]. Three of these corpora have been used in previous research reported in the literature review above: Brown, Bloom, and MPI-EVA-Manchester.
...
third_sg <- read.csv("processed_data/thirdsg_neg.csv")
third_sg <- mutate(third_sg, thirdsg = "Third-singular Utterances")

other_pos <- read.csv("processed_data/other_pos_neg.csv")
...
Specifically, we ask 
(1) what are the syntactic functions of different negators?
(2) what are the common syntactic categories that these negators modify?
(3) how these patterns differ between production of children and that of the parent?
We considered the particular syntactic function of a negator here as being indicated by the syntactic dependency relation between the negator and its syntactic head. And we considered the syntactic category that a negator modifies as being indicated by the POS tag of the negator's syntactic head. 

To do this, we again used the English-North America and English-UK collections of corpora from CHILDES [@macwhinney2000childes], focusing on data with available syntactic parses. Utterances with just one token (excluding punctuation; e.g. *no !*) were removed. Preprocessing led to a total of 3,987,829 syntactically annotated utterances, with 1,930,041 from the English-NA section, and 2,057,788 from the English-UK section. We extracted all utterances that contain any of the three negators, *no*, *not*, and *n't*.
...
For *no*, we first calculated the proportions of its different syntactic functions, as well as the proportions of the different POS tags of its syntactic head. As shown in Figure \@ref(fig:noFunction), the most common dependency relation that *no* bares include (*other* includes all other dependency relations besides the following three):
...
no_function$Dependency <- as.character((no_function$Dependency))
no_function$Dependency[!(no_function$Dependency %in% c('COM', 'QUANT', 'ENUM'))] <- 'Other'

no_function$Dependency <- factor(no_function$Dependency, levels = c('COM', 'QUANT', 'ENUM', 'Other'))

```

```{r noFunction, fig.cap="Comparing the syntactic function of no between children and parent.", fig.env="figure", fig.height=6}
ggplot(no_function, aes(x=Role, y=Proportion, fill=Dependency))+ 
  geom_bar(position="stack",stat='identity',width=0.6)+
  scale_fill_manual(values=c("#0072B2", "#CC79A7", "#009E73", "#999999"))+
...
ggplot(no_communicator_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) + 
  geom_point(size = 3) +
...
ggplot(no_quantifier_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) + 
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
no_quantifier_pos$POS <- as.character(no_quantifier_pos$POS)
no_quantifier_pos$POS[!(no_quantifier_pos$POS %in% c('n', 'pro', 'adj'))] <- 'Other'

no_quantifier_pos$POS <- factor(no_quantifier_pos$POS, levels = c('n', 'pro', 'adj', 'Other'))
...
ggplot(no_quantifier_pos, aes(x=Role, y=Proportion, fill=POS))+ 
  geom_bar(position="stack",stat='identity',width=0.6)+
  scale_fill_manual(values=c("#0072B2", "#CC79A7", "#009E73", "#999999"))+
  labs(x="POS tags")+
  labs(y="Proportion (%)")+
...
We then continued to examine the production of *no* as a quantifier given its most frequent syntactic heads. We focus specifically on cases when the syntactic head of *no* is either a noun or a pronoun, since the numbers of these cases are large enough to yield reliable estimates (>= 1000). In children's production, there are 5,455 utterances where *no* modifies a noun, and 1,100 cases where *no* modifies a pronoun. In parent's production, there are 5,823 utterances where *no* modifies a noun, and 1,018 instances where *no* modifies a pronoun. The two subfigures from Figure \@ref{fig:noQUANTAge} are comparable to each other as well as comparable to cases when *no* was used as a discourse response variable. Parents' usage of *no* as a quantifier modifying a noun or a pronoun is earlier than that of children. On the other hand, children starts using *no* to quantify nouns or pronouns after around 18 months old; their production starts to bare syntactic resemblance to that of parents around the age of 22 months.
...
```{r noQUANTAge, fig.cap="Comparing the trajectory of no as a quantifier of nouns and pronouns between children and parent.", fig.env="figure", fig.height=8}

p1<-ggplot(no_quantifier_n_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) + 
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
#  labs(color = 'Syntactic Position') +
  theme(text = element_text(size = 20),
        legend.position = 'top') +
  scale_x_continuous(breaks=seq(0, 72, 6)) +
  ggtitle('when no is a quantifier of noun')

p2<-ggplot(no_quantifier_pro_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) + 
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
#  labs(color = 'Syntactic Position') +
  theme(text = element_text(size = 20),
        legend.position = 'top') +
  scale_x_continuous(breaks=seq(0, 72, 6)) +
  ggtitle('when no is a quantifier of pronoun')
...
Now we turn to *n't*. Given our corpus search, the linguistic items that *n't* could be combined with is much more restrictive in comparison to *no* and *not*, in the sense that it mostly co-occurs with auxiliary or copula verbs. Therefore we conducted our syntactic analyses with these cases. Following our analyses of *no*, instances with auxiliary or copula verbs that co-occur with *no* for less than 1,000 times were not included. We concentrated on the lexical form of these verbs, therefore for instance, cases with *doesn't* and *don't* were treated as having different auxiliary verbs.
...
for (i in 1:nrow(nt_child)) {
  if (grepl("wouldn't", as.vector(nt_child$Utterance)[i]) & as.vector(nt_child$Head_lexical)[i] %in% c('will')) {
#    child_does = child_does + 1
    nt_child$Head_lexical[i] <- "would"
  }
}
...
nt_lexical <- subset(rbind(nt_child_lexical, nt_parent_lexical), Frequency >= 1000)

```

```{r study3_nt, fig.cap="The most frequent copula and auxiliary verbs that n't combines with.", fig.env="figure", fig.height=6}
...
```{r ntMOST, fig.cap="Comparing the trajectory of nt as a modifier between children and parent.", fig.env="figure", fig.height=16}

p1<-ggplot(nt_most_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
p2<-ggplot(nt_do_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
p3<-ggplot(nt_can_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
p4<-ggplot(nt_be_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
study3_not_pos <- subset(rbind(study3_not_child_pos, study3_not_parent_pos), Frequency >= 1000)

```

```{r notPOS, fig.cap="Comparing the most frequenty syntactic categories of the syntactic head of not between children and parent.", fig.env="figure", fig.height=6}
...
not_child_n_age$Proportion <- not_child_n_age$Frequency * 100 / sum(not_child_n_age$Frequency)
...
not_parent_n_age$Proportion <- not_parent_n_age$Frequency * 100 / sum(not_parent_n_age$Frequency)
...
not_child_adv_age$Proportion <- not_child_adv_age$Frequency * 100 / sum(not_child_adv_age$Frequency)
...
not_parent_adv_age$Proportion <- not_parent_adv_age$Frequency * 100 / sum(not_parent_adv_age$Frequency)
...
not_child_v_age$Proportion <- not_child_v_age$Frequency * 100 / sum(not_child_v_age$Frequency)
...
not_parent_v_age$Proportion <- not_parent_v_age$Frequency * 100 / sum(not_parent_v_age$Frequency)
...
```{r notCOP, fig.cap="Comparing the trajectory of not modifying different syntactic categories, between children and parent.", fig.env="figure", fig.height=18}

p1<-ggplot(not_cop_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
p2<-ggplot(not_pro_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
p3<-ggplot(not_n_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
p4<-ggplot(not_adv_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
p5<-ggplot(not_v_age, aes(x = Age, y = Proportion, shape = Role, color = Role)) +
  geom_point(size = 3) +
  geom_line( color="grey") +
  scale_color_brewer(palette="Dark2") +
#  scale_fill_manual(values=c("#9999CC", "#66CC99"))+
...
*no + XYZ* too noisy data


One crucial note is that for the two negators *not* and *n't*, when they are combined with a copula or an auxiliary verb, they are not necessarily *negating* these copula or auxiliaries. Consider the following two examples:
(1) *it's not a book*
(2) *it isn't a book*
The matter of interest here is what the negator and the copula/auxiliary combined together is modifying, which in this case is the noun phrase *a book*. Furthermore, as can be seen from the two examples above, there are certain cases where *not* and *n't* (together with the copula/auxiliary) occur in similar syntactic contexts. Therefore it remains to be seen to what extent the usage of *not* and *n't* differs when taking into account the linguistic item that the negator and the copula/auxiliary together modifies.

Following our dependency-based analyses, we zoomed in on cases with *copula/auxiliary + not/n't*. For *not*, we focused on utterances where the dependency relation between the negator and the copula/auxiliary is *NEG*. We identified the first dependent to the right of the copular/auxiliary verb that occurs after the negator. Ddue to annotation error, the *first dependent* in some cases were annotated as actually the root copula; we expanded our search to instances as such as well). For instances where the copula/auxiliary and the negator do not modifiy anything else (e.g. *it isn't.*), we left them as they were and did not include them for analyses.

Since we are investigating what *copula/auxiliary + not/n't* together combine with or are modifying, similar to our explorations of *no* and *not*, we again concentrated on the syntactic category of the *first dependent* (e.g. in *it isn't a book*, the syntactic category of *book* is a noun that is the head of a noun phrase (NP)). Then We calculated the probability of each syntactic category. 

The most frequent syntactic category that *copula + not* modifies is *part* (89.77% in 2552 child utterances and 92.14% in 8794 parent utterances) which represents the present or past participle form of a verb (e.g. *I'm not going out the garden*; *you're not taking mine*). Whereas the second most frequent syntactic category is *v* and accounts for only a very small proportion in child (0.06%) and parent (0.04) production. Manual analyses revealed that most instances in the second case are due to annotation error. Therefore we focused on just instances when *copula + not* is modifying *part*.


```{r load data}
study3_not_be <- read.csv('processed_data/study3_not_be.csv', header = T, sep = ',')
child_not_be <- subset(study3_not_be, Role %in% c('Target_Child') & Negation_dep %in% c('NEG') & Head_POS %in% c('cop', 'aux') & !(Center_lemma %in% c('JUST')))
parent_not_be <- subset(study3_not_be, Role %in% c('Parent') & Negation_dep %in% c('NEG') & Head_POS %in% c('cop', 'aux') & !(Center_lemma %in% c('JUST')))

child_not_be_pos <- data.frame(table(child_not_be$Center_POS))
names(child_not_be_pos) <- c('POS', 'Frequency')
child_not_be_pos$POS <- reorder(child_not_be_pos$POS, -child_not_be_pos$Frequency)
child_not_be_pos$Proportion <- child_not_be_pos$Frequency * 100 / sum(child_not_be_pos$Frequency)
child_not_be_pos$Role <- as.vector(rep('Target_Child', nrow(child_not_be_pos)))

parent_not_be_pos <- data.frame(table(parent_not_be$Center_POS))
names(parent_not_be_pos) <- c('POS', 'Frequency')
parent_not_be_pos$POS <- reorder(parent_not_be_pos$POS, -parent_not_be_pos$Frequency)
parent_not_be_pos$Proportion <- parent_not_be_pos$Frequency * 100 / sum(parent_not_be_pos$Frequency)
parent_not_be_pos$Role <- as.vector(rep('Parent', nrow(parent_not_be_pos)))

not_be_pos <- rbind(child_not_be_pos, parent_not_be_pos)

not_be_pos$POS <- as.character(not_be_pos$POS)
not_be_pos$POS[!(not_be_pos$POS %in% c('part', 'v'))] <- 'Other'
not_be_pos$POS <- factor(not_be_pos$POS, levels = c('part', 'v', 'Other'))
...
```{r notCopZoom, fig.cap="Comparing the syntactic function of copula + not between children and parent.", fig.env="figure", fig.height=6}

ggplot(not_be_pos, aes(x=Role, y=Proportion, fill=POS))+ 
  geom_bar(position="stack",stat='identity',width=0.6)+
  scale_fill_manual(values=c("#0072B2", "#CC79A7",  "#999999"))+
  labs(x="POS tags")+
  labs(y="Proportion (%)")+
  scale_y_continuous(limits=c(0,100))+
...
In comparison, in utterances with *copula + n't*, things are different. While cases where *copula + n't* is modifying a participle form of a verb still accounts for a decent proportion (13.16% in 2089 child utterances; 5.96% in 23894 parent utterances), the instances where *copula + n't* modifies a *pro*, which represents pronoun (e.g. *aren't they*), is the most frequent (40.55% in child production and 72.12% in parent production). One main reason behind why the proportion difference of cases where *copula + n't* modifies a  *pro* lies in the fact that most of these instances consist of rhetorical questions (e.g. *there's no pretty aren't they?*).
Overall, there appears to be more variability in child production (2.72) regarding what *copula + n't* is modifying in contrast to that in parent production (1.64).

Low frequency in child production

```{r load data}
study3_nt_be <- read.csv('processed_data/study3_nt_be.csv', header = T, sep = ',')
child_nt_be <- subset(study3_nt_be, Role %in% c('Target_Child') & Negation_dep %in% c('NEG') & Head_POS %in% c('cop', 'aux') & !(Center_lemma %in% c('JUST')))
parent_nt_be <- subset(study3_nt_be, Role %in% c('Parent') & Negation_dep %in% c('NEG') & Head_POS %in% c('cop', 'aux') & !(Center_lemma %in% c('JUST')))

child_nt_be_pos <- data.frame(table(child_nt_be$Center_POS))
names(child_nt_be_pos) <- c('POS', 'Frequency')
child_nt_be_pos$POS <- reorder(child_nt_be_pos$POS, -child_nt_be_pos$Frequency)
child_nt_be_pos$Proportion <- child_nt_be_pos$Frequency * 100 / sum(child_nt_be_pos$Frequency)
child_nt_be_pos$Role <- as.vector(rep('Target_Child', nrow(child_nt_be_pos)))

parent_nt_be_pos <- data.frame(table(parent_nt_be$Center_POS))
names(parent_nt_be_pos) <- c('POS', 'Frequency')
parent_nt_be_pos$POS <- reorder(parent_nt_be_pos$POS, -parent_nt_be_pos$Frequency)
parent_nt_be_pos$Proportion <- parent_nt_be_pos$Frequency * 100 / sum(parent_nt_be_pos$Frequency)
parent_nt_be_pos$Role <- as.vector(rep('Parent', nrow(parent_nt_be_pos)))

nt_be_pos <- rbind(child_nt_be_pos, parent_nt_be_pos)

```

```{r ntCopZoom, fig.cap="Comparing the syntactic function of copula + nt between children and parent.", fig.env="figure", fig.height=6}
ggplot(nt_be_pos, aes(x=POS, y=Proportion, fill=Role))+ 
  geom_bar(position=position_dodge(),stat='identity')+
  scale_fill_manual(values=c("#0072B2", "#CC79A7"))+
  labs(x="Syntactic category (POS)")+
  labs(y="Proportion (%)")+
  scale_y_continuous(limits=c(0,100))+
  theme(text = element_text(size=30))+ 
  theme(legend.position="top")
...
We performed similar analyses for cases with *auxiliary + not/n't*. After removing cases where *auxiliary + not* do not modify any other linguistic item, there were very few instances left in both child (70) and parent (921) production. Admitting the limited sample size here, the most frequent syntactic category that *auxiliary + not* modifies is also the present or past participle form of verbs (*part*), same as instances with *copula + not*; it occurs for 88.57% in child utterances and 84.26% in parent utterances.

In comparison, when the auxiliary verb co-occurs with *n't*, the most frequenty category that *auxiliary + n't* modifies in child production is still participle form of verbs (74.40% in 1164 utterances). Nevertheless, this is not the case in parent production. Although cases where *auxiliary + n't* appears with *part* is quite prevalent (36.53% in 10982 utterances), the most frequent syntactic category that the negator modifies is actually pronouns (55.28%), which is similar to instances with *copula + n't*. In terms of production variability, different from cases with *copula + n't*, there seems to be slightly more variability in parent production (1.46) comapred to child production (1.33).

```{r load data}

study3_not_aux <- read.csv('processed_data/study3_not_aux.csv', header = T, sep = ',')
child_not_aux <- subset(study3_not_aux, Role %in% c('Target_Child') & Negation_dep %in% c('NEG') & Head_POS %in% c('aux') & !(Center_lemma %in% c('JUST')))
parent_not_aux <- subset(study3_not_aux, Role %in% c('Parent') & Negation_dep %in% c('NEG') & Head_POS %in% c('aux') & !(Center_lemma %in% c('JUST')))

child_not_aux_pos <- data.frame(table(child_not_aux$Center_POS))
names(child_not_aux_pos) <- c('POS', 'Frequency')
child_not_aux_pos$POS <- reorder(child_not_aux_pos$POS, -child_not_aux_pos$Frequency)
child_not_aux_pos$Proportion <- child_not_aux_pos$Frequency * 100 / sum(child_not_aux_pos$Frequency)
child_not_aux_pos$Role <- as.vector(rep('Target_Child', nrow(child_not_aux_pos)))

parent_not_aux_pos <- data.frame(table(parent_not_aux$Center_POS))
names(parent_not_aux_pos) <- c('POS', 'Frequency')
parent_not_aux_pos$POS <- reorder(parent_not_aux_pos$POS, -parent_not_aux_pos$Frequency)
parent_not_aux_pos$Proportion <- parent_not_aux_pos$Frequency * 100 / sum(parent_not_aux_pos$Frequency)
parent_not_aux_pos$Role <- as.vector(rep('Parent', nrow(parent_not_aux_pos)))

not_aux_pos <- rbind(child_not_aux_pos, parent_not_aux_pos)

not_aux_pos$POS <- as.character(not_aux_pos$POS)
not_aux_pos$POS[!(not_aux_pos$POS %in% c('part', 'v'))] <- 'Other'
not_aux_pos$POS <- factor(not_aux_pos$POS, levels = c('part', 'v', 'Other'))

```

```{r load data}
study3_nt_aux <- read.csv('processed_data/study3_nt_aux.csv', header = T, sep = ',')
child_nt_aux <- subset(study3_nt_aux, Role %in% c('Target_Child') & Negation_dep %in% c('NEG') & Head_POS %in% c('cop', 'aux') & !(Center_lemma %in% c('JUST')))
parent_nt_aux <- subset(study3_nt_aux, Role %in% c('Parent') & Negation_dep %in% c('NEG') & Head_POS %in% c('cop', 'aux') & !(Center_lemma %in% c('JUST')))

child_nt_aux_pos <- data.frame(table(child_nt_aux$Center_POS))
names(child_nt_aux_pos) <- c('POS', 'Frequency')
child_nt_aux_pos$POS <- reorder(child_nt_aux_pos$POS, -child_nt_aux_pos$Frequency)
child_nt_aux_pos$Proportion <- child_nt_aux_pos$Frequency * 100 / sum(child_nt_aux_pos$Frequency)
child_nt_aux_pos$Role <- as.vector(rep('Target_Child', nrow(child_nt_aux_pos)))

parent_nt_aux_pos <- data.frame(table(parent_nt_aux$Center_POS))
names(parent_nt_aux_pos) <- c('POS', 'Frequency')
parent_nt_aux_pos$POS <- reorder(parent_nt_aux_pos$POS, -parent_nt_aux_pos$Frequency)
parent_nt_aux_pos$Proportion <- parent_nt_aux_pos$Frequency * 100 / sum(parent_nt_aux_pos$Frequency)
parent_nt_aux_pos$Role <- as.vector(rep('Parent', nrow(parent_nt_aux_pos)))

nt_aux_pos <- rbind(child_nt_aux_pos, parent_nt_aux_pos)

entropy <- function(data){
  start = 0
  for (i in 1:nrow(data)){
    p = as.vector(data$Proportion)[i] / 100
#    print(p)
...
```{r ntAUXZoom, fig.cap="Comparing the syntactic function of copula + nt between children and parent.", fig.env="figure", fig.height=6}
ggplot(nt_aux_pos, aes(x=POS, y=Proportion, fill=Role))+ 
  geom_bar(position=position_dodge(),stat='identity')+
  scale_fill_manual(values=c("#0072B2", "#CC79A7"))+
  labs(x="Syntactic category (POS)")+
  labs(y="Proportion (%)")+
