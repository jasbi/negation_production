...
# 
# write.csv(child_utterances, "raw_data/eng_child_utterances.csv")
...
#     {
#       df$condensed_pos[df$clean_utterances == replacements[[i]][[j]]] <- replacements[[i]][[1]]
#     }
#   }
#   return(df)
# }
...
#   filter(!grepl('^no:\\w{2}$|^not$|^[a-z]*n\'t$', condensed_pos))
# 
# #Remove eventually; to show Susan what is removed
# removed_co <- child_negation %>%
...
# 
# 
# write.csv(child_negation, "raw_data/eng_child_negation.csv")

child_negation <- read.csv("raw_data/eng_child_negation.csv")

child_negation$condensed_pos <- as.character(child_negation$condensed_pos)
child_negation$clean_utterances <- as.character(child_negation$clean_utterances)
n_total <- nrow(child_negation)
```
...
#  mutate(type_token_ratio = n_types_clean/n_tokens_clean)
# 
# write.csv(parent_utterances, "raw_data/eng_parent_utterances.csv")

parent_utterances <- read.csv("raw_data/eng_parent_utterances.csv")
# parent_negation <-
#  parent_utterances %>%
#  filter(grepl('( |^)no( |$)|( |^)not( |$)|( |^)[a-z]+n\'t( |$)', clean_utterances)) %>%
#  mutate(can = grepl('^can | can | can$|cannot|can\'t', clean_utterances)) %>%
...
# # Remove utterances that don't line up with their pos
# parent_negation<- parent_negation %>%
#   filter(mapply(check_lengths, condensed_pos, clean_utterances))
# 
# write.csv(parent_negation, "raw_data/eng_parent_negation.csv")

parent_negation <- read.csv("raw_data/eng_parent_negation.csv")

parent_negation$condensed_pos <- as.character(parent_negation$condensed_pos)
parent_negation$clean_utterances <- as.character(parent_negation$clean_utterances)
```
...
# constructions <- count(child_negation$condensed_pos) %>%
#   arrange(desc(freq))
# 
# filtered_constructions <- constructions %>%
#   filter(freq>10, x!="")

### IRRELEVANT SINCE NO BECAME NO:CO AND NO:QN
# table1 <- data.frame(
...
# freq <- function(pos) 
# {
#   filter(child_negation, condensed_pos == pos)$clean_utterances %>% length()
# }
# 
# table1 <- table1 %>%
...
table2 <- data.frame("Category" = c("Can", "Want", "Do","Don't", "Don't want", "Don't know", "Don't like", "No _", "No more", "Not _", "Presentential \`No\'", "Pre-VP \`No\'", "Sentence-internal \`No\'", "Presentential \`Not\'", "Pre-VP \`Not\'", "Sentence-internal \'Not\'"), 
                     "Frequency" = c(length(filter(child_negation, can)$clean_utterances), 
                                     ...
                                     filter(child_negation, grepl('^no [a-z]+$', condensed_pos) & condensed_pos != "no no")$clean_utterances %>% length(),
                                     filter(child_negation, grepl('( |^)no more( |$)', clean_utterances))$clean_utterances %>% length(),
                                     filter(child_negation, grepl('^not [a-z]+$', condensed_pos))$clean_utterances %>% length(),
                                     filter(child_negation, grepl('^no:\\w+ n v ?.*', condensed_pos))$clean_utterances %>% length(),
                                     filter(child_negation, grepl('^no:\\w+ v ?.*', condensed_pos))$clean_utterances %>% length(),
                                     filter(child_negation, grepl('^n no:\\w+ v ?.*', condensed_pos))$clean_utterances %>% length(),
                                     filter(child_negation, grepl('^not n v ?.*', condensed_pos))$clean_utterances %>% length(),
                                     filter(child_negation, grepl('^not v ?.*', condensed_pos))$clean_utterances %>% length(), 
                                     filter(child_negation, grepl('^n not v ?.*', condensed_pos))$clean_utterances %>% length()), 
...
table2 <- table2 %>%
  mutate(Percent_total =(Frequency/n_total) * 100)

table2 <- table2[c(1, 2, 4, 3)]
...
filter(child_negation, grepl('^no .*', condensed_pos) & !grepl('^(no ?)+$', condensed_pos))$clean_utterances %>% length()

#filters for 'no n [anything]', excluding repeated 'no'; 236 utterances (6.75%)
filter(child_negation, grepl('^no n.*', condensed_pos) & !grepl('^(no ?)+$', condensed_pos))$clean_utterances %>% length()
...
child_utterances$part_of_speech <- as.character(child_utterances$part_of_speech)
child_utterances$clean_utterances <- as.character(child_utterances$clean_utterances)
...
#  mutate(no_other = str_count(condensed_pos, '( |^)no:[^(co|qn)]+( |$)')) %>%
...
pre_no_tag <- function(pos) {
  if(grepl('no:\\w+ (det )?n v ?.*', pos)) {return("presentential")}
  else if (grepl('n no:\\w+ v ?.*', pos)){return("pre-verb (w/ subject)")}
  else if (grepl('no:\\w+ v ?.*', pos)){return("pre-verb (w/o subject)")}
  else if (grepl('no:\\w+ (det )?n ?.*', pos)){return("pre-noun")}
  else {return("other")}
}

pre_no <- child_negation %>%
  filter(grepl('( |^)no:\\w+( |$)', condensed_pos)) %>%
  mutate(no = lapply(condensed_pos, pre_no_tag))

pre_no$no <- factor(pre_no$no, levels = c("presentential", "pre-verb (w/ subject)", "pre-verb (w/o subject)","pre-noun", "other"))
...
pre_not_tag <- function(pos) {
  if(grepl('not (det )?n v ?.*', pos)) {return("presentential")}
  else if (grepl('n not v ?.*', pos)){return("pre-verb (w/ subject)")}
  else if (grepl('not v ?.*', pos)){return("pre-verb (w/o subject)")}
  else if (grepl('not (det )?n ?.*', pos)){return("pre-noun")}
  else {return("other")}
}

pre_not <- child_negation %>%
  filter(grepl('( |^)not( |$)', condensed_pos)) %>%
  mutate(not = lapply(condensed_pos, pre_not_tag))

pre_not$not <- factor(pre_not$not, levels = c("presentential", "pre-verb (w/ subject)", "pre-verb (w/o subject)","pre-noun", "other"))
