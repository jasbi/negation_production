...
child_utterances <- child_utterances %>%
  mutate(n_tokens_clean = factor(n_tokens_clean, levels = append(as.character(1:21), c(">20"))))

child_utterances$n_tokens_clean[is.na(child_utterances$n_tokens_clean)] <- ">20"
...
neg_proportions <- (neg_sums[,-(1:2)] / neg_sums[,2]) %>%
...
ggplot(neg_proportions_melted) +
  ...
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("PARENT Negation type: All Multi-word Negative Utterances")+
  scale_fill_brewer(palette="Paired")+
  theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 70, vjust = 1, hjust=1))
...
```{r child no:co composition, echo=FALSE}
# No:co composition
...
no_cos_prop <- merge(
  tally(group_by(no_cos, bin)) %>% dplyr::rename(total = n), 
  dcast(no_cos, bin~no_co, value.var = "no_co", fun.aggregate = length), by = "bin")
# Divide by total to get proportion by bin
no_cos_prop[,-(1:2)] <- no_cos_prop[,-(1:2)]/no_cos_prop[,2]

# Prepare for graphing
no_cos_prop <- no_cos_prop[,-2] %>%
  melt(id = "bin", value.name = "proportion", variable.name = "PoS")

# Ensure same order of elements
no_cos_prop <- no_cos_prop[order(rev(no_cos_prop$bin),-no_cos_prop$proportion),]
order <- unique(no_cos_prop$PoS)
no_cos_prop$PoS <- factor(no_cos_prop$PoS, levels = order)
no_cos$no_co <- factor(no_cos$no_co, levels = order)
...
ggplot() +
  ...
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("CHILD \'no (co)\' Composition")+
...
ggplot() +
  geom_bar(no_cos_prop, mapping = aes(bin, proportion, fill = PoS), stat ="identity")+
  xlab("Age Bin")+
...
```{r parent no:co composition, echo=FALSE}
...
p_no_cos_prop <- merge(
  ...
  dcast(no_cos, bin~no_co, value.var = "no_co", fun.aggregate = length), by = "bin")
# Divide by total to get proportion by bin
p_no_cos_prop[,-(1:2)] <-p_no_cos_prop[,-(1:2)]/p_no_cos_prop[,2]

# Prepare for graphing
p_no_cos_prop <- p_no_cos_prop[,-2] %>%
  melt(id = "bin", value.name = "proportion", variable.name = "PoS")

# Ensure same order of elements
p_no_co_order <- append(as.character(order), as.character(filter(p_no_cos_prop, !(PoS %in% order))$PoS %>% unique()))
p_no_cos_prop$PoS <- factor(p_no_cos_prop$PoS, levels = p_no_co_order)
p_no_cos$no_co <- factor(p_no_cos$no_co, levels = order)
...
ggplot() +
  ...
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("PARENT \'no (co)\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
  theme(legend.key=element_blank(), legend.key.size=unit(12,"point"))+
  labs(fill = "Part of Speech") +
  scale_fill_manual(values=as.vector(alphabet2()))
...
ggplot() +
  geom_bar(p_no_cos_prop, mapping = aes(bin, proportion, fill = PoS), stat ="identity")+
  xlab("Age Bin")+
  ylab("Proportion of Instances")+
  ggtitle("PARENT \'no (co)\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
...
```{r child no:qn composition, echo=FALSE}
# No:qn composition
...
# Create proportion table
no_qns_prop <- merge(
  tally(group_by(no_qns, bin)) %>% dplyr::rename(total = n), 
  dcast(no_qns, bin~no_qn, value.var = "no_qn", fun.aggregate = length), by = "bin")
# Divide by total to get proportion by bin
no_qns_prop[,-(1:2)] <- no_qns_prop[,-(1:2)]/no_qns_prop[,2]

# Prepare for graphing
no_qns_prop <- no_qns_prop[,-2] %>%
  melt(id = "bin", value.name = "proportion", variable.name = "PoS")

# Ensure same order of elements
no_qn_order <- append(as.character(order), as.character(filter(no_cos_prop, !(PoS %in% order))$PoS %>% unique()))
no_qns_prop$PoS <- factor(no_qns_prop$PoS, levels = no_qn_order)
no_qns$no_qn <- factor(no_qns$no_qn, levels = no_qn_order)
...
ggplot() +
  ...
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("CHILD \'no (qn)\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
  theme(legend.key=element_blank(), legend.key.size=unit(12,"point"))+
  labs(fill = "Part of Speech") +
  scale_fill_manual(values=as.vector(alphabet2()))
...
ggplot() +
  geom_bar(no_qns_prop, mapping = aes(bin, proportion, fill = PoS), stat ="identity")+
  xlab("Age Bin")+
  ylab("Proportion of Instances")+
  ggtitle("CHILD \'no (qn)\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
...
```{r parent no:qn composition, echo=FALSE}
...
p_no_qns_prop <- merge(
  ...
  dcast(no_qns, bin~no_qn, value.var = "no_qn", fun.aggregate = length), by = "bin")
# Divide by total to get proportion by bin
p_no_qns_prop[,-(1:2)] <-p_no_qns_prop[,-(1:2)]/p_no_qns_prop[,2]

# Prepare for graphing
p_no_qns_prop <- p_no_qns_prop[,-2] %>%
  melt(id = "bin", value.name = "proportion", variable.name = "PoS")

# Ensure same order of elements
p_no_qn_order <- append(as.character(p_no_co_order), as.character(filter(p_no_qns_prop, !(PoS %in% p_no_co_order))$PoS %>% unique()))
p_no_qns_prop$PoS <- factor(p_no_qns_prop$PoS, levels = p_no_qn_order)
...
ggplot() +
  ...
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("PARENT \'no (qn)\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
  theme(legend.key=element_blank(), legend.key.size=unit(12,"point"))+
  labs(fill = "Part of Speech") +
  scale_fill_manual(values=as.vector(alphabet2()))
...
ggplot() +
  geom_bar(p_no_qns_prop, mapping = aes(bin, proportion, fill = PoS), stat ="identity")+
  xlab("Age Bin")+
  ylab("Proportion of Instances")+
  ggtitle("PARENT \'no (qn)\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
...
```{r parent all no composition, include=FALSE}
...
```{r child not composition, echo=FALSE}
...
# Ensure same order of elements
not_extras <- unique(nots_prop$PoS)[!unique(nots_prop$PoS) %in% order]
not_order <- c(as.character(order), as.character(not_extras))
...
```{r parent not composition, echo=FALSE}
...
# Create proportion table
nots_p_prop <- merge(
  tally(group_by(nots_p, bin)) %>% dplyr::rename(total = n), 
  dcast(nots_p, bin~not, value.var = "not", fun.aggregate = length), by = "bin")
# Divide by total to get proportion by bin
nots_p_prop[,-(1:2)] <- nots_p_prop[,-(1:2)]/nots_p_prop[,2]

# Prepare for graphing
nots_p_prop <- nots_p_prop[,-2] %>%
  melt(id = "bin", value.name = "proportion", variable.name = "PoS")

# Ensure same order of elements
not_extras_p <- unique(nots_p_prop$PoS)[!unique(nots_p_prop$PoS) %in% not_order]
not_p_order <- c(as.character(not_order), as.character(not_extras_p))
nots_p_prop$PoS <- factor(nots_p_prop$PoS, levels = not_p_order)
nots_p$not <- factor(nots_p$not, levels = not_p_order)

ggplot(nots_p, aes(bin, fill = not)) +
  geom_bar()+
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("PARENT \'not\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
  theme(legend.key=element_blank(), legend.key.size=unit(12,"point"))+
  labs(fill = "Part of Speech") +
  scale_fill_manual(values=as.vector(alphabet2()))

ggplot(nots_p_prop, aes(bin, proportion, fill = PoS)) +
  geom_bar(stat ="identity")+
  xlab("Age Bin")+
  ylab("Proportion of Instances")+
  ggtitle("PARENT \'not\' Composition")+
...
```{r child mod composition, echo=FALSE}
...
# Ensure same order of elements
mod_extras <- unique(mods_prop$PoS)[!unique(mods_prop$PoS) %in% order]
mod_order <- c(as.character(order), as.character(mod_extras))
...
```{r parent mod composition, echo=FALSE}
...
# Create proportion table
mods_p_prop <- merge(
  tally(group_by(mods_p, bin)) %>% dplyr::rename(total = n), 
  dcast(mods_p, bin~mod, value.var = "mod", fun.aggregate = length), by = "bin")
# Divide by total to get proportion by bin
mods_p_prop[,-(1:2)] <- mods_p_prop[,-(1:2)]/mods_p_prop[,2]

# Prepare for graphing
mods_p_prop <- mods_p_prop[,-2] %>%
  melt(id = "bin", value.name = "proportion", variable.name = "PoS")

# Ensure same order of elements
mod_p_extras <- unique(mods_p_prop$PoS)[!unique(mods_p_prop$PoS) %in% mod_order]
mod_p_order <- c(as.character(mod_order), as.character(mod_p_extras))
mods_p_prop$PoS <- factor(mods_p_prop$PoS, levels = mod_p_order)
mods_p$mod <- factor(mods_p$mod, levels = mod_p_order)

ggplot(mods_p, aes(bin, fill = mod)) +
  geom_bar()+
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("PARENT \'mod\' Composition")+
  theme(axis.text.x = element_text(angle = 70, vjust = 1, hjust=1), plot.title = element_text(hjust = 0.5))+
  theme(legend.key=element_blank(), legend.key.size=unit(12,"point"))+
  labs(fill = "Part of Speech") +
  scale_fill_manual(values=as.vector(alphabet2()))

ggplot(mods_p_prop, aes(bin, proportion, fill = PoS)) +
  geom_bar(stat ="identity")+
  xlab("Age Bin")+
  ylab("Proportion of Instances")+
  ggtitle("PARENT \'mod\' Composition")+
...
# Ensure same order of elements
nt_extras <- unique(nts_prop$PoS)[!unique(nts_prop$PoS) %in% order]
nt_order <- c(as.character(order), as.character(nt_extras))
...
# Create proportion table
nts_p_prop <- merge(
  tally(group_by(nts_p, bin)) %>% dplyr::rename(total = n), 
  dcast(nts_p, bin~nt, value.var = "nt", fun.aggregate = length), by = "bin")
# Divide by total to get proportion by bin
nts_p_prop[,-(1:2)] <- nts_p_prop[,-(1:2)]/nts_p_prop[,2]

# Prepare for graphing
nts_p_prop <- nts_p_prop[,-2] %>%
  melt(id = "bin", value.name = "proportion", variable.name = "PoS")

# Ensure same order of elements_p
nt_p_extras <- unique(nts_p_prop$PoS)[!unique(nts_p_prop$PoS) %in% nt_order]
nt_p_order <- c(as.character(nt_order), as.character(nt_p_extras))
nts_p_prop$PoS <- factor(nts_p_prop$PoS, levels = nt_p_order)
nts_p$nt <- factor(nts_p$nt, levels = nt_p_order)
...
ggplot(nts_p_prop, aes(bin, proportion, fill = PoS)) +
  geom_bar(stat ="identity")+
  xlab("Age Bin")+
  ylab("Proportion of Instances")+
  ggtitle("PARENT \'nt\' Composition")+
