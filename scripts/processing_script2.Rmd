...
child_utterances$n_tokens_clean <- factor(child_utterances$n_tokens_clean, levels = as.character(1:20))
ggplot(child_utterances, aes(bin, fill = n_tokens_clean))+
  geom_bar() +
  ggtitle("Utterance Length")+
...
```{r child no composition, echo=FALSE}
# No composition
no_tag <- function(pos) 
{
  pos_list = unlist(strsplit(pos, " "))
...
no_order <- unique(nos_prop$PoS)
nos_prop$PoS <- factor(nos_prop$PoS, levels = no_order)
nos$no <- factor(nos$no, levels = no_order)

ggplot() +
  geom_bar(nos, mapping = aes(bin, fill = no))+
  xlab("Age Bin")+
  ylab("Number of Instances")+
  ggtitle("CHILD \'No\' Composition")+
...
ggplot() +
  geom_bar(nos_prop, mapping = aes(bin, proportion, fill = PoS), stat ="identity")+
  xlab("Age Bin")+
...
```{r parent no composition, echo=FALSE}
...
# Ensure same order of elements
not_extras <- unique(nots_prop$PoS)[!unique(nots_prop$PoS) %in% no_order]
not_order <- c(as.character(no_order), as.character(not_extras))
...
# Ensure same order of elements
mod_extras <- unique(mods_prop$PoS)[!unique(mods_prop$PoS) %in% no_order]
mod_order <- c(as.character(no_order), as.character(mod_extras))
...
# Ensure same order of elements
nt_extras <- unique(nts_prop$PoS)[!unique(nts_prop$PoS) %in% no_order]
nt_order <- c(as.character(no_order), as.character(nt_extras))
