...
pre_no_tag <- function(pos, gloss) {
  if(grepl('no:\\w+( det)?(( adv)* adj)* n (adv )*(v|part|mod|cop)( adv)*', pos)) {
    if(grepl('( |^)no .*n\'t', gloss) | grepl('( |^)no (.* )?not?', gloss)){return("no + NP + VP (NEG)")}
    else return("no + NP + VP")}
  else if (grepl('(det )?((adv )*adj )*n (adv )*(v|part|mod|cop)( adv)* .*no:co$', pos)& !grepl('(say|said)( oh)? no', gloss)) {
    if(grepl('*n\'t( .*)* no( |$)', gloss) | grepl('not( .*)* no( |$)', gloss)){return("NP + VP + no (NEG)")}
    else return("NP + VP + no")}
...
pre_not_tag <- function(pos, gloss) {
  if(grepl('not( det)?(( adv)* adj)* n (adv )*(v|part|mod|cop)( adv)*', pos)) {return("not + NP + VP")}
  else if (grepl('(det )?((adv )*adj )*n (adv )*(v|part|mod|cop)( adv)* .*not$', pos)& !grepl('(say|said)( oh)? no', gloss))
    {return("NP + VP + not")}
...
not_3sg <- filter(child_neg_no_imperatives, grepl(paste(pronouns3sg, 'not( adv)? v', sep = " "), v_utterances)) %>%
  mutate(neg = "not")
nt_3sg <- filter(child_neg_no_imperatives, grepl(paste(pronouns3sg, '(don\'t|doesn\'t)( adv)? v', sep = " "), v_utterances)) %>%
  mutate(neg = "nt")
